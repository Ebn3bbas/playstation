  ...
  if (endedSessions && endedSessions?.rows?.length !== 0) {
    for (let r in endedSessions.rows) {
      endedData.push({
        ...
        started_at:
          endedSessions?.rows[r]?.started_at?.substr(11, 8) || "no data",
        ended_at:
          endedSessions?.rows[r]?.ended_at?.substr(11, 8) || "not ended yet",
        edit: (
          <EditButton
            updateHandler={() => updateHandler(endedSessions?.rows[r]?.id)}
          />
        ),
        delete: (
          <DeleteButton
            deleteHandler={() => deleteHandler(endedSessions?.rows[r]?.id)}
          />
        ),
      });
    }
  ...
  if (notEndedSessions && notEndedSessions?.rows?.length !== 0) {
    for (let r in notEndedSessions.rows) {
      data.push({
        ...
        started_at:
          notEndedSessions?.rows[r]?.started_at?.substr(11, 8) || "no data",

        add_orders: (
          <PrimaryButton
            title="Add Orders"
            onClick={() =>
              navigate(`/sessions-add-orders/${notEndedSessions?.rows[r]?.id}`)
            }
          />
        ),
        edit: (
          <EditButton
            updateHandler={() => updateHandler(notEndedSessions?.rows[r]?.id)}
          />
        ),
        delete: (
          <DeleteButton
            deleteHandler={() => deleteHandler(notEndedSessions?.rows[r]?.id)}
          />
        ),
      });
    }
  ...
  return (
    <div className={styles.RoomsContainer}>
      <div className={styles.RoomsTable}>
        ...
        {deleteSuccess && <Success>Seesion deleted successfully</Success>}
        {loading ? (
          <p>loading...</p>
        ) : (
          <div>
